...
| 2026-08-29 | Netatmo active-rain read-only check | During user-observed rain at 12:20:53Z, `sensor.netatmo_home_rain_sensor_rain_last_hour` was fresh at `0.303 mm`; direct `..._rain` was `0.101 mm`; `binary_sensor.is_it_raining` was `on`; and daily total was `2.5 mm`. All four last changed at 13:13:15 BST (12:13:15Z), so the source was actively reporting rather than stale. The mower was docked. The repository’s `rain_accumulation.py`, `ground_dry`, and reviewed entity names are not yet deployed: their expected HA entities were absent. | This is a valid fresh source observation supporting D-03’s rolling-hour binding and supplies another rain-onset timestamp for the latency log. It cannot validate GoMow wetness accumulation, dry-down, active-job dock, or post-rain resume behaviour because those GoMow modules are not deployed and no mower command was issued. Direct `..._rain` is now fresh but remains excluded from GoMow until its independent semantics are explicitly evidenced. |
